...
namespace KGC\RdvBundle\Elastic\Provider;
 ...
 * Class RdvProvider.
 *
 * @DI\Service("kgc.elastic.rdv.provider")
 * @DI\Tag("fos_elastica.provider", attributes = {"index" = "kgestion_idx", "type" = "rdv"})
 */
class RdvProvider implements ProviderInterface
{
    /**
     ...
     *
     * @DI\InjectParams({
     *      "objectPersister" = @DI\Inject("fos_elastica.object_persister.kgestion_idx.rdv"),
    ...
    public function populate(\Closure $loggerClosure = null, array $options = array())
    {
        ...
        $repository = $em->getRepository('KGCRdvBundle:RDV');
        $queryBuilder = $repository->createQueryBuilderIndex('rdv')
            ->orderBy('rdv.id', 'ASC')
            ->setMaxResults($batchSize)
            ->andWhere('rdv.id > :fromid');

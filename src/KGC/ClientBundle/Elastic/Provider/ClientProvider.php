...
namespace KGC\ClientBundle\Elastic\Provider;
 ...
 *
 * @DI\Service("kgc.elastic.client.provider")
 * @DI\Tag("fos_elastica.provider", attributes = {"index" = "kgestion_idx", "type" = "client"})
 */
class ClientProvider implements ProviderInterface
{
     ...
     *
     * @DI\InjectParams({
     *      "objectPersister" = @DI\Inject("fos_elastica.object_persister.kgestion_idx.client"),
    ...
    public function populate(\Closure $loggerClosure = null, array $options = array())
    {
        ...
        $batchSize = isset($options['batch_size']) ? intval($options['batch_size']) : 1000;

        $em = $this->doctrine->getManager();
        $repository = $em->getRepository('KGCSharedBundle:Client');
        $queryBuilder = $repository->createQueryBuilderIndex('client')
            ->orderBy('client.id', 'ASC')
            ->setMaxResults($batchSize)
            ->andWhere('client.id > :fromid');
